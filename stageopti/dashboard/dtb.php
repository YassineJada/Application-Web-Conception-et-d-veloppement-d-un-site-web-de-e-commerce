...
class daoges{
    ...
    function returnallclientinfo(){
        $bdd = $this->connexion();
        $log = $bdd->query("SELECT * FROM tclient ");
    ...
    function nbcommand($email){
        $bdd = $this->connexion();
        $log = $bdd->query("SELECT * FROM commande WHERE cemail = '$email'");
    ...
    function addquanti($idpro,$quan){
        $bdd = $this->connexion();
        $respose = $bdd->query("UPDATE  produit  SET quantité = quantité + $quan WHERE pid = $idpro  ");

    }

    function retour($numcommand){
        $bdd = $this->connexion();
        $respose1 = $bdd->query("UPDATE  commande  SET cstatus ='retour' WHERE numcomand = $numcommand   ");

        $respose = $bdd->query("SELECT * FROM commande  NATURAL JOIN produit WHERE commande.prodpid = produit.pid AND numcomand = $numcommand ");     

        while($ligne= $respose->fetch()){
            $this->addquanti($ligne['prodpid'],$ligne['pquantité']);
        }
    ...
    function addprude($ptitre,$pcategorie,$quantité,$discount,$prixV,$prixA,$ptype,$desci,$tva){
        $bdd = $this->connexion();
        $respose = $bdd->query("INSERT INTO produit Values (NULL,'$ptitre','$pcategorie',$quantité,$discount,$prixV,$prixA,'$ptype','$desci',$tva) ");
    ...
    function updateproduite($pid,$ptitre,$pcategorie,$quantité,$discount,$prixV,$prixA,$ptype,$desci,$tva){
        $bdd = $this->connexion();
        $respose = $bdd->query("UPDATE produit SET ptitre = '$ptitre' , pcategorie = '$pcategorie' , quantité='$quantité',discount='$discount', prixV= $prixV , prixA= $prixA , ptype = '$ptype' ,desci='$desci', tva =$tva WHERE pid = '$pid'");
        echo "UPDATE produit SET ptitre = '$ptitre' , pcategorie = '$pcategorie' , quantité='$quantité',discount='$discount', prixV= $prixV , prixA= $prixA , ptype = '$ptype' ,desci='$desci', tva =$tva WHERE pid = '$pid'";
    }
    ...
    function listeprudect($pid){
        ...
        if($pid == ""){
            ...
            while($ligne= $respose->fetch()){
                $liste[]=[$ligne['pid'],$ligne['ptitre'],$ligne['pcategorie'],$ligne['quantité'],$ligne['discount'],$ligne['prixV'],$ligne['prixA'],$ligne['ptype'],$ligne['desci'],$ligne['tva']];
            }
    ...
    function lowquantitépro($nb){
        $bdd = $this->connexion(); 
        
        if($nb == 0){
            $respose = $bdd->query("SELECT * from produit Where quantité = 0 ");

        }else{
            $respose = $bdd->query("SELECT * from produit Where quantité < 5 AND quantité > 0 ");
